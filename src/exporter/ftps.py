...
import settings
import argparse
from locations import WAVEFORM_PSEUDONYMISED
...
def do_upload_inner(rel_file_to_upload: Path):
    ...
    WAVEFORM_PSEUDONYMISED.mkdir(parents=False, exist_ok=True)
    file_to_upload = (WAVEFORM_PSEUDONYMISED / rel_file_to_upload).resolve(strict=True)
    # Check that even after evaluating ".." and symlinks, the file is still under the "safe" directory
    # for upload.
    if not file_to_upload.is_relative_to(WAVEFORM_PSEUDONYMISED):
        raise ValueError(f"File {file_to_upload} must be under {WAVEFORM_PSEUDONYMISED}. "
